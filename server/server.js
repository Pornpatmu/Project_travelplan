...
mockPlaces.forEach((place) => {
  db.get(`SELECT * FROM all_places WHERE name = ?`, [place.name], (err, row) => {
    if (!row) {
      db.run(
        `INSERT INTO all_places (name, lat, lon, category, type, province, image, openingDays, phone, description, address, hours, mapUrl)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          place.name,
          place.lat,
...
`;
  ...
  db.all(query, [province, category], (err, rows) => {
    ...
    if (rows.length === 0) {
      console.warn('⚠️ ไม่พบข้อมูลจากเงื่อนไข province/category ที่ระบุ');
      return res.status(404).json({ error: 'ไม่พบสถานที่' });
    }

    const shuffled = rows.sort(() => 0.5 - Math.random());
    const selected = shuffled.slice(0, Math.min(rows.length, 5));
...
app.get('/places', (req, res) => {
  ...
  let query = `SELECT * FROM all_places WHERE TRIM(LOWER(province)) = TRIM(LOWER(?))`;
  ...
  console.log(`👉 GET /places: province='${province}', type='${type || 'ALL'}'`);
});
