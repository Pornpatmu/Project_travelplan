const express = require('express');
const sqlite3 = require('sqlite3').verbose();
const cors = require('cors');

const app = express();
const port = 3000;

app.use(cors());
app.use(express.json());

// --- เปิด database ---
const db = new sqlite3.Database('./mydatabase.db');

// --- สร้างตารางต่างๆ 
db.serialize(() => {
  // ตารางแผนเที่ยว
  db.run(`
    CREATE TABLE IF NOT EXISTS plans (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT UNIQUE,  
  province TEXT,
  start_date TEXT,
  end_date TEXT,
  budget REAL,
  spending REAL,
  dayColors TEXT
)

  `);

  // ตารางสถานที่ในแผนแบบ minimal:
  // เปลี่ยนจากการเก็บข้อมูลเต็ม (name, lat, lon) เป็นเก็บเฉพาะ place_id
  db.run(`
    CREATE TABLE IF NOT EXISTS places (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      plan_id INTEGER,
      day_index INTEGER,
      place_id INTEGER,
      expense REAL,
      order_index INTEGER,
      category TEXT
    )
  `);

  db.run(`
    CREATE TABLE IF NOT EXISTS favorite_places (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      plan_id INTEGER,
      name TEXT,
      lat REAL,
      lon REAL,
      category TEXT
    )
  `);

  db.run(`
    CREATE TABLE IF NOT EXISTS other_expenses (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      plan_id INTEGER,
      desc TEXT,
      amount REAL,
      icon_code INTEGER
    )
  `);
});

db.run(`
  CREATE TABLE IF NOT EXISTS all_places (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT,
    lat REAL,
    lon REAL,
    category TEXT,
    province TEXT,
    image TEXT,
    openingDays TEXT,
    phone TEXT
  )
`);

// ---  เพิ่มข้อมูล mockPlaces ลงใน all_places  ---
const mockPlaces = [
  {
    'name': 'Blue Lagoon',
    'lat': 16.66506,
    'lon': 101.83263,
    'category': 'nature',
    'province': 'ขอนแก่น',
    'image': 'assets/images/BlueLagoon.jpg',
    'description': 'สระน้ำสีมรกตขนาดใหญ่จากการทำเหมืองหินเก่าที่มีบรรยากาศธรรมชาติและเงียบสงบ',
    'address': 'บ้านสองคอน ตำบลนาฝาย อำเภอภูผาม่าน จังหวัดขอนแก่น',
    'hours': '06:00 - 19:00 น.',
    'phone': '-',
    'mapUrl': 'https://goo.gl/maps/yagD2DtF2C3VKyy88',
  },
  {
    'name': 'น้ำตกตาดฟ้า',
    'lat': 16.79155,
    'lon': 102.25934,
    'category': 'nature',
    'province': 'ขอนแก่น',
    'image': 'assets/images/Namtok.jpg',
    'description': 'น้ำตกที่สูงถึง 15 เมตรในอุทยานแห่งชาติภูเวียง จังหวัดขอนแก่น',
    'address': 'ตำบลภูเวียง อำเภอภูเวียง จังหวัดขอนแก่น',
    'hours': '08:30 - 16:30 น.',
    'phone': '0-4343-8333',
    'mapUrl': 'https://goo.gl/maps/DeVan5dGf1fnzV4BA',
  },
  {
    'name': 'จุดชมทิวทัศน์หินช้างสี อุทยานแห่งชาติน้ำพอง',
    'lat': 16.65329,
    'lon': 102.60846,
    'category': 'nature',
    'province': 'ขอนแก่น',
    'image': 'assets/images/ScenicView.jpg',
    'description': 'จุดชมวิวที่สามารถมองเห็นอ่างเก็บน้ำเขื่อนอุบลรัตน์และตัวเมืองขอนแก่นแบบพาโนรามา',
    'address': 'ตำบลโคกงาม อำเภอบ้านฝาง จังหวัดขอนแก่น',
    'hours': '08:00 - 16:30 น.',
    'phone': '096 739 7920',
    'mapUrl': 'https://goo.gl/maps/QKXkY4Du4QrrYt3k9',
  },
  {
    'name': 'บึงทุ่งสร้าง',
    'lat': 16.45262,
    'lon': 102.85490,
    'category': 'nature',
    'province': 'ขอนแก่น',
    'image': 'assets/images/BuengThungSang.jpg',
    'description': 'บึงทุ่งสร้างเป็นสวนสาธารณะที่เหมาะสำหรับการออกกำลังกายและพักผ่อนหย่อนใจ',
    'address': 'ถนนจอมพล ตำบลในเมือง อำเภอเมืองขอนแก่น จังหวัดขอนแก่น',
    'hours': 'ตลอดทั้งวัน',
    'phone': '-',
    'mapUrl': 'https://maps.app.goo.gl/aSRpuKukiZuuA9kF7',
  },
  {
    'name': 'บางแสน2ขอนแก่น',
    'lat': 16.73959,
    'lon': 102.62847,
    'category': 'nature',
    'province': 'ขอนแก่น',
    'image': 'assets/images/BangSaen2.jpg',
    'description': 'ทะเลน้ำจืดในขอนแก่นที่เป็นแหล่งพักผ่อนหย่อนใจของชาวขอนแก่น',
    'address': 'ตำบลเขื่อนอุบลรัตน์ อำเภออุบลรัตน์ จังหวัดขอนแก่น',
    'hours': '09:00 - 18:00 น.',
    'phone': '-',
    'mapUrl': 'https://maps.app.goo.gl/m5eMAmhMviYke4DS7',
  },
  {
    'name': 'เขื่อนอุบลรัตน์',
    'lat': 16.77371,
    'lon': 102.61922,
    'category': 'nature',
    'province': 'ขอนแก่น',
    'image': 'assets/images/UbolratanaDam.jpg',
    'description': 'เขื่อนขนาดใหญ่ในจังหวัดขอนแก่นที่เหมาะสำหรับการพักผ่อนและชมพระอาทิตย์ตก',
    'address': 'อำเภออุบลรัตน์ จังหวัดขอนแก่น',
    'hours': '08:00 – 16:30 น.',
    'phone': '-',
    'mapUrl': 'https://maps.app.goo.gl/i7s6KxXbdLHXh3iS8',
  },
  {
    'name': 'Little Box Café',
    'lat': 16.78427,
    'lon': 102.62954,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/LittleBoxCafe.jpg',
    'description': 'คาเฟ่วิวสวยที่มีมุมถ่ายรูปเยอะมาก และมีเมนูเครื่องดื่มราคาไม่แพงแถมตอบโจทย์สายเซลฟี่ได้เป็นอย่างดี',
    'address': 'ตำบลบ้านดง อำเภออุบลรัตน์ จังหวัดขอนแก่น',
    'hours': '08:00 - 17:30 น.',
    'phone': '088 744 7356',
    'mapUrl': 'https://maps.app.goo.gl/S1eDDSu2cpBacJfd8',
  },
  {
    'name': 'หมู่บ้านงานคราฟต์',
    'lat': 16.47914,
    'lon': 102.80553,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/CraftVillageCafe.jpg',
    'description': 'หมู่บ้านเล็ก ๆ ที่ซ่อนตัวอยู่หลังมหาวิทยาลัยขอนแก่น ที่มีทั้งคาเฟ่ ร้านหนังสือ และร้านปั้นเซรามิก',
    'address': 'หลังมหาลัยขอนแก่น 674 บ้านโนนม่วง ม.19 ต.ศิลา อ.เมือง จ.ขอนแก่น',
    'hours': '09:00 - 23:00 น.',
    'phone': '085 083 7635',
    'mapUrl': 'https://goo.gl/maps/gJ6ZKMwioh6qGdSK8',
  },
  {
    'name': 'กระต่ายขูดมะพร้าว',
    'lat': 16.46514,
    'lon': 102.82504,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/RabbitCafe.jpg',
    'description': 'คาเฟ่ที่เสิร์ฟขนมหวานหลากหลายเมนูในรูปแบบใหม่ของขนมไทย',
    'address': 'โซนกังสดาลมข. ถ.กัลปพฤกษ์ ข้างๆร้านสยามนิสิต จ.ขอนแก่น',
    'hours': '13:00 - 21:30 น.',
    'phone': '080 329 8883',
    'mapUrl': 'https://maps.app.goo.gl/EvAzF5GNVSsS4fxz8?g_st=ic',
  },
  {
    'name': 'นอนนา คาเฟ่ Khon Kaen',
    'lat': 16.54368,
    'lon': 102.84756,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/NonnaCafe.jpg',
    'description': 'คาเฟ่ทุ่งนาในบรรยากาศที่ดีและมีมุมถ่ายรูปสุดปัง พร้อมเมนูอาหารที่หลากหลาย',
    'address': 'ม.6 บ้านไก่นา ต.สำราญ อ.เมือง จ.ขอนแก่น',
    'hours': '09:00 - 22:00 น.',
    'phone': '092-478-4789',
    'mapUrl': 'https://goo.gl/maps/nSXBJG7TME7jxa7g8',
  },
  {
    'name': 'Rak An Coffee',
    'lat': 16.43069,
    'lon': 102.82957,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/RakAnCoffee.jpg',
    'description': 'คาเฟ่เก่าแก่ที่นำเสนอเมนูขนมหวานและกาแฟที่ใช้เมล็ดกาแฟหลากหลายพันธุ์',
    'address': '8 ซอยดรุณสำราญ ใกล้ศาลหลักเมืองขอนแก่น ทางไปสถานีรถไฟ',
    'hours': '08:30 - 17:00 น. (จันทร์-ศุกร์), 09:00 – 18:00 น. (เสาร์-อาทิตย์)',
    'phone': '095 661 0199',
    'mapUrl': 'https://maps.app.goo.gl/Dvkob6KPeJYtTJ6Y6',
  },
  {
    'name': 'Goodbeans Roastery',
    'lat': 16.45806,
    'lon': 102.82497,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/GoodbeansRoastery.jpg',
    'description': 'คาเฟ่บรรยากาศสวยและร่มรื่น ใช้การตกแต่งแบบปูนเปลือยและงานไม้สไตล์มินิมอลญี่ปุ่น',
    'address': 'ถนน ตะวันใหม่ อำเภอเมืองขอนแก่น จังหวัดขอนแก่น',
    'hours': '07:30 - 18:30 น.',
    'phone': '082 449 1728',
    'mapUrl': 'https://maps.app.goo.gl/WQ5vPV75Ff31Ps1K9',
  },
  {
    'name': 'บ้านแนบน้ำ NABNAM SPACE',
    'lat': 16.44293,
    'lon': 102.90582,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/NABNAMSpace.jpg',
    'description': 'คาเฟ่ริมน้ำที่มีมุมถ่ายรูปฟีลดีทุกมุม และเมนูเครื่องดื่มและขนมให้เลือกมากมาย',
    'address': 'หมู่ 12 บ้านสงเปือย ถนน ขอนแก่นเชียงยืน อำเภอเมืองขอนแก่น จังหวัดขอนแก่น',
    'hours': '09:00 - 18:00 น.',
    'phone': '089 710 8382',
    'mapUrl': 'https://maps.app.goo.gl/wzqLX1kuo5GMbYBDA?g_st=ic',
  },
  {
    'name': 'Chujai the roaster',
    'lat': 16.43832,
    'lon': 102.82897,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/ChujaiTheRoaster.jpg',
    'description': 'ร้านกาแฟบรรยากาศโฮมมี่ที่เป็น Pet friendly และยังมีเมนูอาหารสุดอร่อย',
    'address': 'ตำบลในเมือง อำเภอเมืองขอนแก่น ขอนแก่น 40000',
    'hours': '08:30 - 19:00 น.',
    'phone': '096 687 5356',
    'mapUrl': 'https://maps.app.goo.gl/wXaRwc95DsrQueXS9?g_st=ic',
  },
  {
    'name': 'Punpang (ปั้นแป้ง)',
    'lat': 16.42689,
    'lon': 102.80205,
    'category': 'cafe',
    'province': 'ขอนแก่น',
    'image': 'assets/images/PunpangCafe.jpg',
    'description': 'ร้านขนมหวานโฮมเมดที่มีมุมถ่ายรูปเพียบ เหมาะสำหรับสาว ๆ ที่ชอบการถ่ายภาพ',
    'address': '577 ถนน บ้านกอก 40 ตำบลบ้านเป็ด อำเภอเมืองขอนแก่น ขอนแก่น 40000',
    'hours': '11:00 - 20:00 น.',
    'phone': '-',
    'mapUrl': 'https://maps.app.goo.gl/ucRM5CXYai2uzsJV9?g_st=ic',
  },
  {
    'name': 'ล่องแก่งผาเทวดา',
    'lat': 16.65148, // พิกัดละติจูดที่ใกล้เคียง
    'lon': 101.77447, // พิกัดลองติจูดที่ใกล้เคียง
    'category': 'adventure',  // สายผจญภัย
    'province': 'ขอนแก่น',
    'image': 'assets/images/PaThewadaRafting.jpg',
    'description': 'สถานที่ท่องเที่ยวที่เหมาะกับการพายเรือชมวิวระยะทาง 9 กม. ธรรมชาติสวยงาม มีน้ำเชี่ยวและน้ำดูดระยะทางแต่ละช่วง',
    'address': 'บ้านเขาวง อ.ภูผาม่าน ขอนแก่น ประเทศไทย',
    'hours': '08:30 - 17:00 น.',
    'phone': '094 556 6416',
    'mapUrl': 'https://maps.app.goo.gl/xmnVKWk8udSCPzZA9',
    'entryFee': 'คนละ 250 บาท',
  },
  {
    'name': 'แม่บัวไล ณ ขอนแก่น Farm Stay',
    'lat': 16.2705,
    'lon': 102.8816,
    'category': 'adventure',  // สายผจญภัย
    'province': 'ขอนแก่น',
    'image': 'assets/images/MaBualaiFarmStay.jpg',
    'description': 'สถานที่ท่องเที่ยวแห่งใหม่ที่ให้คุณได้สัมผัสวิถีชีวิตของชุมชนและทำกิจกรรมต่างๆ เช่น การใช้จักรยานเที่ยวและกินอาหารท้องถิ่น',
    'address': 'บ้านโนนข่า อ.พล จ.ขอนแก่น',
    'hours': 'ทุกวัน',
    'phone': '-',
    'mapUrl': '-',  // ไม่ได้ระบุลิงค์ Google Maps
    'entryFee': '-',
  },
  {
    'name': 'อุทยานแห่งชาติภูผาม่าน',
    'lat': 16.75701,
    'lon': 101.96990,
    'category': 'adventure',  // สายผจญภัย
    'province': 'ขอนแก่น',
    'image': 'assets/images/PhuPhamarnNationalPark.jpg',
    'description': 'อุทยานแห่งชาติที่มีแหล่งท่องเที่ยวทางธรรมชาติหลากหลาย เช่น ภูเขา ถ้ำพญานาคราช น้ำตกตาดใหญ่ และน้ำตกพลาญทอง',
    'address': '201 ตำบลนาหนองทุ่ม อำเภอชุมแพ จังหวัดขอนแก่น',
    'hours': '08:30 – 16:30 น.',
    'phone': '043-001753',
    'mapUrl': 'https://maps.app.goo.gl/cbqTuYoz6gJjS8e67',
    'entryFee': 'ผู้ใหญ่ (ชาวไทย) 20 บาท, เด็ก (ชาวไทย) 10 บาท / ผู้ใหญ่ (ต่างชาติ) 100 บาท, เด็ก (ต่างชาติ) 50 บาท',
  },
  {
    'name': 'อนุสาวรีย์พระนครศรีบริรักษ์',
    'lat': 16.42077,
    'lon': 102.83855,
    'category': 'history',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/PhraNakhonSriBhirakStatue.jpg',
    'description': 'อนุสาวรีย์พระนครศรีบริรักษ์หรือท้าวเพี้ยเมืองแพน ขุนนางที่มีบทบาทสำคัญในการสร้างเมืองขอนแก่นในปี พ.ศ. 2340',
    'address': 'ถนนรอบบึงแก่นนคร ตำบลเมืองเก่า อำเภอเมืองขอนแก่น จังหวัดขอนแก่น',
    'hours': '06:00 - 18:00 น.',
    'phone': '-',
    'mapUrl': 'https://maps.app.goo.gl/3L2isp64cYM4GC459',
    'entryFee': 'ฟรี',
  },
  {
    'name': 'ปราสาทเปือยน้อย',
    'lat': 15.88028,
    'lon': 102.90969,
    'category': 'history',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/PrasatPueanNoi.jpg',
    'description': 'ปราสาทศิลปะขอมโบราณผสมระหว่างศิลปะเขมรแบบบาปวนและนครวัด สร้างในพุทธศตวรรษที่ 16-17 เป็นปราสาทที่มีความสมบูรณ์ที่สุดในภาคอีสานตอนบน',
    'address': 'ตำบลเปือยน้อย อำเภอเปือยน้อย จังหวัดขอนแก่น',
    'hours': '07:00 - 19:00 น.',
    'phone': '062 119 5789',
    'mapUrl': 'https://goo.gl/maps/tfDmmZQks9aAz5Zk8',
    'entryFee': 'ฟรี',
  },
  {
    'name': 'กู่ประภาชัย',
    'lat': 16.61225,
    'lon': 102.93129,
    'category': 'history',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/KuPraphachai.jpg',
    'description': 'โบราณสถานที่สร้างขึ้นตั้งแต่สมัยพระเจ้าชัยวรมันที่ 7 แห่งอาณาจักรเขมรโบราณ ภายในมีพระพุทธรูปประดิษฐานอยู่และหินศักดิ์สิทธิ์ให้กราบไหว้',
    'address': 'หมู่ 1 ตำบลบัวใหญ่ อำเภอน้ำพอง จังหวัดขอนแก่น',
    'hours': '06:00 - 18:00 น.',
    'phone': '-',
    'mapUrl': 'https://goo.gl/maps/hyUTKjyfevU9k3ro6',
    'entryFee': 'ฟรี',
  },
  {
    'name': 'เทวาลัยศิวะมหาเทพ',
    'lat': 16.46497,
    'lon': 102.89064,
    'category': 'history',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/ShivaMahatepTemple.jpg',
    'description': 'ศาสนสถานของศาสนาฮินดูที่สร้างขึ้นตั้งแต่ปี พ.ศ. 2554 มีองค์พระพิฆเนศ อัมรินทราธิราชเจ้า และพระตรีมูรติ เป็นต้น',
    'address': 'ถนนเลี่ยงเมืองขอนแก่น อำเภอเมืองขอนแก่น จังหวัดขอนแก่น',
    'hours': '08:00 - 18:00 น.',
    'phone': '095 959 8789',
    'mapUrl': 'https://maps.app.goo.gl/rSRgzr6sitTgJWyn9',
    'entryFee': 'ฟรี',
  },
  {
    'name': 'โสกผีดิบ',
    'lat': 15.83322,
    'lon': 102.69289,
    'category': 'history',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/SokPheeDib.jpg',
    'description': 'แหล่งท่องเที่ยวทางธรณีวิทยาที่มีอายุประมาณ 65 ล้านปี เป็นลักษณะหินยุคครีเทเซียสตอนปลายที่โดนกัดเซาะจากกระแสน้ำ',
    'address': 'บ้านหนองบัวน้อย หมู่ 8 ตำบลโสกนกเต็น อำเภอพล จังหวัดขอนแก่น',
    'hours': 'ทุกวัน',
    'phone': '-',
    'mapUrl': 'https://goo.gl/maps/VQ2GEdaH3yba7EtB8',
    'entryFee': 'ฟรี',
  },
  {
    'name': 'กู่แก้ว',
    'lat': 16.42594,
    'lon': 102.48080,
    'category': 'history',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/KuKaew.jpg',
    'description': 'โบราณสถานในเมืองขอนแก่นที่เป็นรูปปราสาทหินแบบอโรคยาศาล ตกแต่งในศิลปะแบบบายน จากสมัยพระเจ้าชัยวรมันที่ 7',
    'address': 'วัดกู่แก้วรัตนาราม บ้านหัวสระ ตำบลดอนช้าง อำเภอเมือง จังหวัดขอนแก่น',
    'hours': 'ตลอดวัน',
    'phone': '-',
    'mapUrl': 'https://goo.gl/maps/g9aVjpveemP6kshK9',
    'entryFee': 'ฟรี',
  },
  {
    'name': 'พิพิธภัณฑ์ไดโนเสาร์ภูเวียง',
    'lat': 16.67870,
    'lon': 102.26843,
    'category': 'family',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/DinosaurMuseum.jpg',
    'description': 'พิพิธภัณฑ์ที่จัดแสดงซากฟอสซิลไดโนเสาร์และเรื่องราวเกี่ยวกับไดโนเสาร์ในภูเวียง',
    'address': 'ตำบลในเมือง อำเภอเวียงเก่า จังหวัดขอนแก่น',
    'hours': '09:00 – 17:00 น.',
    'phone': '0-4343-8204',
    'mapUrl': 'https://goo.gl/maps/MB8RmfJGkXQ313wn7',
    'entryFee': 'คนไทย ผู้ใหญ่ 20 บาท เด็ก 10 บาท, ต่างชาติ ผู้ใหญ่ 60 บาท เด็ก 30 บาท',
  },
  {
    'name': 'สวนสัตว์ขอนแก่น',
    'lat': 16.84570,
    'lon': 102.89725,
    'category': 'family',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/KhonKaenZoo.jpg',
    'description': 'สถานที่พักผ่อนหย่อนใจที่มีสัตว์ต่างๆ จากทั่วโลกและกิจกรรมให้อาหารสัตว์และการแสดง',
    'address': 'ตำบลคำม่วง อำเภอเขาสวนกวาง จังหวัดขอนแก่น',
    'hours': '08:00 – 16:30 น.',
    'phone': '-',
    'mapUrl': 'https://maps.app.goo.gl/do74oxcvUxW6AZYB7',
    'entryFee': 'ผู้ใหญ่ 130 บาท เด็ก 40 บาท',
  },
  {
    'name': 'พระธาตุขามแก่น',
    'lat': 16.56411,
    'lon': 102.95404,
    'category': 'family',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/PhraThatKhamKaen.jpg',
    'description': 'ปูชนียสถานที่สำคัญที่มีประวัติยาวนาน และยังเป็นตราสัญลักษณ์ของจังหวัดขอนแก่น',
    'address': 'วัดเจติยภูมิ บ้านขาม หมู่ที่ 1 ตำบลบ้านขาม อำเภอน้ำพอง จังหวัดขอนแก่น',
    'hours': '07:00 – 17:00 น.',
    'phone': '-',
    'mapUrl': 'https://goo.gl/maps/ohvuGXatxPXiZ9Je7',
    'entryFee': 'ฟรี',
  },

  {
    'name': 'วัดพระพุทธบาทภูพานคำ',
    'lat': 16.76296,
    'lon': 102.62371,
    'category': 'family',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/PhraPhutthabatPhuPankham.jpg',
    'description': 'วัดที่ประดิษฐานรอยพระพุทธบาทจำลองและพระพุทธอุตรมหามงคลอุบลรัตน์, บันไดพญานาค 1,049 ขั้น',
    'address': '103 หมู่ 1 อำเภออุบลรัตน์ จังหวัดขอนแก่น',
    'hours': '07:00 - 17:00 น.',
    'phone': '-',
    'mapUrl': 'https://goo.gl/maps/vGXL3yogyFCz9DrK7',
    'entryFee': 'ฟรี',
  },
  {
    'name': 'เอ๊กซอติค เพท แอนด์ ฟาวเทน โชว์',
    'lat': 16.45983,
    'lon': 102.79495,
    'category': 'family',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/ExoticPetsFountainShow.jpg',
    'description': 'สวนสัตว์ขนาดเล็กที่มีสัตว์หลากหลายชนิดและการแสดงน้ำพุเริงระบำในช่วงกลางคืน',
    'address': '888 ซอยนวลหง ตำบลศิลา อำเภอเมืองขอนแก่น ขอนแก่น',
    'hours': '10:00 - 18:00 น.',
    'phone': '088-192-1555',
    'mapUrl': 'https://maps.app.goo.gl/ia9Fih9v6z5FrCA58',
    'entryFee': '150 บาท',
  },
  {
    'name': 'วัดทุ่งเศรษฐี',
    'lat': 16.41210,
    'lon': 102.88589,
    'category': 'family',  // สายประวัติศาสตร์
    'province': 'ขอนแก่น',
    'image': 'assets/images/WatThungSeththi.jpg',
    'description': 'วัดที่มีมหาเจดีย์ผสมผสานระหว่างองค์เจดีย์จากทั้งสามโลก, และมีจุดสักการะมากมาย',
    'address': 'ตำบลพระลับ อำเภอเมือง จังหวัดขอนแก่น',
    'hours': '07:00 - 17:00 น.',
    'phone': '-',
    'mapUrl': 'https://goo.gl/maps/TgwSuiuKF9gaiSqK8',
    'entryFee': 'ฟรี',
  },
  {
    'name': 'ตลาดต้นตาล',
    'lat': 16.45186,
    'lon': 102.82163,
    'category': 'nightlife',  // สายกลางคืน
    'province': 'ขอนแก่น',
    'image': 'assets/images/TonTanMarket.jpg',
    'description': 'ตลาดกลางคืนที่ใหญ่ที่สุดในขอนแก่น มีหลากหลายโซนให้เลือกเดิน เช่น โซนอาหาร, เครื่องดื่ม, เสื้อผ้า, และของฝาก รวมทั้งกิจกรรมต่าง ๆ เช่น โซนดูดวงและลานเบียร์',
    'address': 'ถ. มิตรภาพ ตำบลในเมือง อำเภอเมืองขอนแก่น ขอนแก่น 40110',
    'hours': '16:00 – 23:00 น.',
    'phone': '083 284 5351',
    'mapUrl': 'https://maps.app.goo.gl/MXT3dpw9pAKzLekQ6',
    'entryFee': '-',
  },
  {
    'name': 'ตลาดจอมพล',
    'lat': 16.45186,
    'lon': 102.84191,
    'category': 'nightlife',  // สายกลางคืน
    'province': 'ขอนแก่น',
    'image': 'assets/images/ChomPhonMarket.jpg',
    'description': 'ตลาดที่มีของกินแซ่บ ๆ มากมาย เช่น ร้านอาหารตามสั่งและขนมหวาน รวมถึงร้านบุฟเฟต์และสินค้าต่าง ๆ เช่น เสื้อผ้าและเครื่องสำอาง',
    'address': '555 ถ.กสิกรทุ่งสร้าง ต.ในเมือง อ.เมืองขอนแก่น จ.ขอนแก่น',
    'hours': '07:00 – 23:00 น.',
    'phone': '081 661 3300',
    'mapUrl': 'https://maps.app.goo.gl/JLahcsLjmMgs147r6',
    'entryFee': '-',
  },
  {
    'name': 'ตลาด 62 BLOC Market',
    'lat': 16.45744,
    'lon': 102.82425,
    'category': 'nightlife',  // สายกลางคืน
    'province': 'ขอนแก่น',
    'image': 'assets/images/62BLOCMarket.jpg',
    'description': 'ตลาดกลางคืนยอดนิยมที่มีของกินอร่อย ๆ และเป็นแหล่งพักผ่อนหย่อนใจสำหรับนักเรียน นักศึกษา และวัยทำงาน',
    'address': '157/89-91 ถนน กัลปพฤกษ์ อำเภอเมืองขอนแก่น ขอนแก่น 40000',
    'hours': '12:00 - 22:00 น.',
    'phone': '083 662 2619',
    'mapUrl': 'https://maps.app.goo.gl/sr9HdXhWYi2kfMuUA',
    'entryFee': '-',
  },
  {
    'name': 'ตลาดโต้รุ่ง รื่นรมย์',
    'lat': 16.42556,
    'lon': 102.83318,
    'category': 'nightlife',  // สายกลางคืน
    'province': 'ขอนแก่น',
    'image': 'assets/images/RuenRomMarket.jpg',
    'description': 'ตลาดโต้รุ่งที่ตั้งอยู่บนถนนรื่นรมย์ ในช่วงกลางคืนบรรยากาศคึกคัก มีร้านค้าเรียงรายเต็มถนนและทั้งอาหารและข้าวของเครื่องใช้ให้เลือกมากมาย',
    'address': '4 รื่นรมย์ ตำบลในเมือง อำเภอเมืองขอนแก่น ขอนแก่น 40000',
    'hours': '17:00 - 02:00 น.',
    'phone': '-',
    'mapUrl': 'https://maps.app.goo.gl/6Vb41MDBqSAXpcHU8',
    'entryFee': '-',
  },
  {
    name: 'วัดหนองแวง',
    lat: 16.4471,
    lon: 102.8355,
    category: 'family',
    province: 'ขอนแก่น',
    image: 'assets/images/NongWaengTemple.jpg',
    description: 'วัดที่เหมาะสำหรับครอบครัว เป็นที่ตั้งของพระมหาธาตุแก่นนคร',
    address: 'ถ.กลางเมือง ต.ในเมือง อ.เมือง จ.ขอนแก่น',
    hours: '06:00 - 18:00 น.',
    phone: '043-222-111',
    mapUrl: 'https://goo.gl/maps/example123',
  }

];


mockPlaces.forEach((place) => {
  db.get(`SELECT * FROM all_places WHERE name = ?`, [place.name], (err, row) => {
    if (!row) {
      db.run(
        `INSERT INTO all_places (name, lat, lon, category, province, image, openingDays, phone, description, address, hours, mapUrl)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          place.name,
          place.lat,
          place.lon,
          place.category.trim(),
          place.province.trim(),
          place.image || null,
          place.openingDays || place.hours || null,
          place.phone || null,
          place.description || null,
          place.address || null,
          place.hours || null,
          place.mapUrl || null
        ]
      );
    }
  });
});

//random places
app.get('/places/random', (req, res) => {
  const province = req.query.province;
  const category = req.query.category;

  console.log(`[REQ] 🔍 province=${province} | category=${category}`);

  if (!province || !category) {
    return res.status(400).json({ error: 'Missing province or category' });
  }


  const query = `
  SELECT * FROM all_places 
WHERE TRIM(LOWER(province)) = TRIM(LOWER(?))
  AND TRIM(LOWER(category)) = TRIM(LOWER(?))
`;

  console.log(query);

  db.all(query, [province, category], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    console.log(`✅ rows.length = ${rows.length}`); // <--- เพิ่มตรงนี้


    if (rows.length === 0)
      return res.status(404).json({ error: 'ไม่พบสถานที่' });

    const shuffled = rows.sort(() => 0.5 - Math.random());
    const selected = shuffled.slice(0, Math.min(rows.length, 5)); // ได้ตั้งแต่ 1–5
    res.json(selected);

    console.log(`[BACKEND] QUERY: province=${province}, category=${category}`);

  });
});


// --- API ดึงสถานที่จากจังหวัด ---
app.get('/places', (req, res) => {
  const province = req.query.province?.trim();
  const category = req.query.category?.trim();
  db.all(
    `SELECT * FROM all_places WHERE province = ?`,
    [province],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(rows);
    }
  );
  console.log(`👉 GET /places/random: province='${province}', category='${category}'`);

});

// --- ข้อมูลพิกัดจังหวัดภาคอีสาน ---
const coords = {
  'กาฬสินธุ์': { lat: 16.4313, lon: 103.5060 },
  'ขอนแก่น': { lat: 16.4322, lon: 102.8236 },
  'ชัยภูมิ': { lat: 15.8067, lon: 102.0315 },
  'นครพนม': { lat: 17.4108, lon: 104.7784 },
  'นครราชสีมา': { lat: 14.9799, lon: 102.0977 },
  'บึงกาฬ': { lat: 18.3609, lon: 103.6465 },
  'บุรีรัมย์': { lat: 14.9946, lon: 103.1036 },
  'มหาสารคาม': { lat: 16.1970, lon: 103.2830 },
  'มุกดาหาร': { lat: 16.5453, lon: 104.7230 },
  'ยโสธร': { lat: 15.7940, lon: 104.1453 },
  'ร้อยเอ็ด': { lat: 16.0567, lon: 103.6531 },
  'เลย': { lat: 17.4860, lon: 101.7220 },
  'สกลนคร': { lat: 17.1620, lon: 104.1476 },
  'สุรินทร์': { lat: 14.8818, lon: 103.4936 },
  'ศรีสะเกษ': { lat: 15.1148, lon: 104.3294 },
  'หนองคาย': { lat: 17.8783, lon: 102.7420 },
  'หนองบัวลำภู': { lat: 17.2220, lon: 102.4260 },
  'อำนาจเจริญ': { lat: 15.8642, lon: 104.6258 },
  'อุดรธานี': { lat: 17.4138, lon: 102.7872 },
  'อุบลราชธานี': { lat: 15.2448, lon: 104.8473 }
};
// API ดึงสถานที่จาก place_id
app.get('/places/:id', (req, res) => {
  const id = req.params.id;
  db.get(`SELECT * FROM all_places WHERE id = ?`, [id], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!row) return res.status(404).json({ error: 'ไม่พบสถานที่' });
    res.json(row);
  });
});

// --- API ดึงชื่อจังหวัดทั้งหมด (เฉพาะชื่อ) ---
app.get('/provinces', (req, res) => {
  res.json(Object.keys(coords));
});

// --- API ดึงพิกัดจังหวัด ---
app.get('/province/location', (req, res) => {
  const provinceName = req.query.name;
  if (coords[provinceName]) {
    res.json({ name: provinceName, ...coords[provinceName] });
  } else {
    res.status(404).json({ error: 'ไม่พบจังหวัดที่ระบุ' });
  }
});

// --- API แผนทั้งหมด ---
app.get('/plans', (req, res) => {
  db.all('SELECT * FROM plans ORDER BY id DESC', [], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(rows);
  });
});

// --- API สร้างแผนใหม่ --- 
app.post('/plans', (req, res) => {
  const {
    name, province, start_date, end_date, budget, spending,
    dayColors, favoritePlaces, placesByDay, otherExpenses
  } = req.body;

  db.run(
    `INSERT INTO plans (name, province, start_date, end_date, budget, spending, dayColors)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [name, province, start_date, end_date, budget, spending, JSON.stringify(dayColors || {})],
    function (err) {
      if (err) {
        if (err.message.includes("UNIQUE constraint failed")) {
          return res.status(400).json({ error: 'มีชื่อแผนนี้อยู่แล้ว กรุณาใช้ชื่ออื่น' });
        }
        return res.status(500).json({ error: err.message });
      }

      const planId = this.lastID;

      // --- เพิ่ม favoritePlaces ---
      if (Array.isArray(favoritePlaces)) {
        const favStmt = db.prepare(
          `INSERT INTO favorite_places (plan_id, name, lat, lon, category) VALUES (?, ?, ?, ?, ?)`
        );
        favoritePlaces.forEach((fav) => {
          favStmt.run(planId, fav.name, fav.lat, fav.lon, fav.category);
        });
        favStmt.finalize();
      }

      // --- เพิ่ม placesByDay ---
      if (placesByDay && typeof placesByDay === 'object') {
        const placeStmt = db.prepare(
          `INSERT INTO places (plan_id, day_index, place_id, expense, order_index, category)
           VALUES (?, ?, ?, ?, ?, ?)`
        );
        Object.entries(placesByDay).forEach(([day, places]) => {
          places.forEach((place) => {
            placeStmt.run(planId, day, place.place_id, place.expense || 0, place.order_index || 0, place.category);
          });
        });
        placeStmt.finalize();
      }

      // --- ✅ เพิ่ม otherExpenses ---
      if (Array.isArray(otherExpenses)) {
        const expStmt = db.prepare(
          `INSERT INTO other_expenses (plan_id, desc, amount, icon_code)
           VALUES (?, ?, ?, ?)`
        );
        otherExpenses.forEach((exp) => {
          expStmt.run(planId, exp.desc, exp.amount, exp.icon_code || null);
        });
        expStmt.finalize();
      }

      // ✅ ส่ง id กลับ
      res.status(201).json({ id: planId });
    }
  );
});



// --- ✅ API ดึงแผนตาม id (พร้อม places, favorites, expenses) ---
// ใช้ JOIN กับ all_places เพื่อดึงข้อมูลรายละเอียดเต็มของสถานที่
app.get('/plans/:id', (req, res) => {
  const planId = req.params.id;

  db.get('SELECT * FROM plans WHERE id = ?', [planId], (err, plan) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!plan) return res.status(404).json({ error: 'ไม่พบแผนที่ต้องการ' });

    const placesQuery = `
      SELECT 
        p.day_index, 
        p.expense, 
        p.order_index,
        p.category,
        ap.id AS place_id,
        ap.name AS place_name,
        ap.lat,
        ap.lon,
        ap.image,
        ap.openingDays,
        ap.phone
      FROM places p 
      LEFT JOIN all_places ap ON p.place_id = ap.id 
      WHERE p.plan_id = ?
      ORDER BY p.day_index, p.order_index
    `;

    db.all(placesQuery, [planId], (err, places) => {
      if (err) return res.status(500).json({ error: err.message });

      const groupedPlaces = {};
      for (const place of places) {
        // หาก record (place) เป็น null หรือไม่มีค่า day_index ให้ข้าม record นั้น
        if (!place || place.day_index == null) {
          continue;
        }
        if (!groupedPlaces[place.day_index]) {
          groupedPlaces[place.day_index] = [];
        }
        groupedPlaces[place.day_index].push(place);
      }

      db.all('SELECT * FROM favorite_places WHERE plan_id = ?', [planId], (err, favorites) => {
        if (err) return res.status(500).json({ error: err.message });

        db.all('SELECT * FROM other_expenses WHERE plan_id = ?', [planId], (err, expenses) => {
          if (err) return res.status(500).json({ error: err.message });

          res.json({
            id: plan.id,
            name: plan.name,
            province: plan.province,
            start_date: plan.start_date,
            end_date: plan.end_date,
            budget: plan.budget,
            spending: plan.spending,
            dayColors: plan.dayColors ? JSON.parse(plan.dayColors) : {},
            favoritePlaces: favorites,
            placesByDay: groupedPlaces,
            otherExpenses: expenses,
          });
        });
      });
    });
  });
});

// --- ✅ API PUT อัปเดตแผนเที่ยว ---
app.put('/plans/:id', (req, res) => {
  const planId = req.params.id;
  const {
    name, province, start_date, end_date, budget, spending,
    dayColors, favoritePlaces, placesByDay, otherExpenses
  } = req.body;

  db.run(
    `UPDATE plans  SET name=?, province=?, start_date=?, end_date=?, budget=?, spending=? WHERE id=?`,
    [name, province, start_date, end_date, budget, spending, planId],
    function (err) {
      if (err) {
        console.error(err);
        return res.status(500).json({ error: 'Failed to update plan' });
      }

      // ✅ ลบของเก่าออกก่อน (กันข้อมูลซ้ำ)
      db.run(`DELETE FROM favorite_places WHERE plan_id = ?`, [planId]);
      db.run(`DELETE FROM places  WHERE plan_id = ?`, [planId]);
      db.run(`DELETE FROM other_expenses WHERE plan_id = ?`, [planId]);

      // ✅ favoritePlaces
      if (Array.isArray(favoritePlaces)) {
        favoritePlaces.forEach(place => {
          db.run(
            `INSERT INTO favorite_places (plan_id, name, lat, lon, category)
             VALUES (?, ?, ?, ?, ?)`,
            [planId, place.name, place.lat, place.lon, place.category]
          );
        });

      }

      // ✅ placesByDay
      if (placesByDay && typeof placesByDay === 'object') {
        Object.entries(placesByDay).forEach(([dayIndex, places]) => {
          places.forEach(place => {
            db.run(
              `INSERT INTO places  (plan_id, day_index, place_id, expense, order_index)
               VALUES (?, ?, ?, ?, ?)`,
              [planId, dayIndex, place.place_id, place.expense || 0, place.order_index || 0]
            );
          });
        });
      }

      // ✅ otherExpenses
      if (Array.isArray(otherExpenses)) {
        otherExpenses.forEach(exp => {
          db.run(
            `INSERT INTO other_expenses (plan_id, desc, amount, icon_code)
             VALUES (?, ?, ?, ?)`,
            [planId, exp.desc, exp.amount, exp.icon_code || null]
          );
        });
      }

      // ✅ optional: save dayColors as JSON
      db.run(
        `UPDATE plans SET dayColors = ? WHERE id = ?`,
        [JSON.stringify(dayColors || {}), planId]
      );


      res.json({ success: true });
    }
  );
});


// --- ✅ API POST เพิ่มสถานที่ในแผน ---
// รับข้อมูล minimal: plan_id, day_index, place_id, expense, order_index และ category
app.post('/places', (req, res) => {
  const { plan_id, day_index, place_id, expense, order_index, category } = req.body;
  db.run(
    `INSERT INTO places (plan_id, day_index, place_id, expense, order_index, category)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [plan_id, day_index, place_id, expense, order_index, category],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ id: this.lastID });
    }
  );
});

// --- ✅ API POST เพิ่มสถานที่โปรด ---
app.post('/favorites', (req, res) => {
  const { plan_id, name, lat, lon, category } = req.body;
  db.run(
    `INSERT INTO favorite_places (plan_id, name, lat, lon, category)
     VALUES (?, ?, ?, ?, ?)`,
    [plan_id, name, lat, lon, category],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ id: this.lastID });
    }
  );
});

// --- ✅ API POST เพิ่มค่าใช้จ่ายอื่น ๆ ---
app.post('/expenses', (req, res) => {
  const { plan_id, desc, amount, icon_code } = req.body;
  db.run(
    `INSERT INTO other_expenses (plan_id, desc, amount, icon_code)
     VALUES (?, ?, ?, ?)`,
    [plan_id, desc, amount, icon_code],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ id: this.lastID });
    }
  );
});

// --- ✅ API DELETE สถานที่ในแผน (optional ใช้กับปุ่มลบ) ---
app.delete('/places/:id', (req, res) => {
  const placeId = req.params.id;
  db.run('DELETE FROM places WHERE id = ?', [placeId], function (err) {
    if (err) return res.status(500).json({ error: err.message });
    res.status(200).json({ message: 'ลบสถานที่สำเร็จ' });
  });
});

// --- ✅ API PUT อัปเดตงบประมาณและรายการใช้จ่ายเพิ่มเติม ---
app.put('/plans/:id', (req, res) => {
  const planId = req.params.id;
  const { name, budget, spending, dayColors, favoritePlaces, placesByDay, otherExpenses } = req.body;

  if (!name || name.trim() === '') {
    return res.status(400).json({ error: 'กรุณาระบุชื่อแผน' });
  }

  db.serialize(() => {
    db.run("BEGIN TRANSACTION");

    // 1. อัปเดตข้อมูลหลักในตาราง plans
    db.run(
      `UPDATE plans SET name = ?, budget = ?, spending = ?, dayColors = ? WHERE id = ?`,
      [name, budget, spending, JSON.stringify(dayColors), planId],
      function (err) {
        if (err) {
          db.run("ROLLBACK");
          return res.status(500).json({ error: 'ไม่สามารถอัปเดตแผนได้: ' + err.message });
        }

        // 2. อัปเดต favorite_places
        db.run(`DELETE FROM favorite_places WHERE plan_id = ?`, [planId], (err) => {
          if (err) {
            db.run("ROLLBACK");
            return res.status(500).json({ error: 'ไม่สามารถลบ favorite_places: ' + err.message });
          }

          if (favoritePlaces && Array.isArray(favoritePlaces) && favoritePlaces.length > 0) {
            const favStmt = db.prepare(
              `INSERT INTO favorite_places (plan_id, name, lat, lon, category) VALUES (?, ?, ?, ?, ?)`
            );
            favoritePlaces.forEach(fav => {
              favStmt.run(planId, fav.name, fav.lat, fav.lon, fav.category);
            });
            favStmt.finalize((err) => {
              if (err) {
                db.run("ROLLBACK");
                return res.status(500).json({ error: 'ไม่สามารถเพิ่ม favorite_places: ' + err.message });
              }
            });
          }

          // 3. อัปเดต places (สำหรับ placesByDay)
          db.run(`DELETE FROM places WHERE plan_id = ?`, [planId], (err) => {
            if (err) {
              db.run("ROLLBACK");
              return res.status(500).json({ error: 'ไม่สามารถลบ places: ' + err.message });
            }

            if (placesByDay && typeof placesByDay === 'object') {
              for (let dayKey in placesByDay) {
                const dayRecords = placesByDay[dayKey];
                if (Array.isArray(dayRecords) && dayRecords.length > 0) {
                  const placeStmt = db.prepare(
                    `INSERT INTO places (plan_id, day_index, place_id, expense, order_index, category)
                     VALUES (?, ?, ?, ?, ?, ?)`
                  );
                  dayRecords.forEach(record => {
                    placeStmt.run(planId, dayKey, record.place_id, record.expense, record.order_index, record.category);
                  });
                  placeStmt.finalize((err) => {
                    if (err) {
                      db.run("ROLLBACK");
                      return res.status(500).json({ error: 'ไม่สามารถเพิ่ม places: ' + err.message });
                    }
                  });
                }
              }
            }

            // 4. อัปเดต other_expenses
            db.run(`DELETE FROM other_expenses WHERE plan_id = ?`, [planId], (err) => {
              if (err) {
                db.run("ROLLBACK");
                return res.status(500).json({ error: 'ไม่สามารถลบ other_expenses: ' + err.message });
              }

              if (otherExpenses && Array.isArray(otherExpenses) && otherExpenses.length > 0) {
                const expStmt = db.prepare(
                  `INSERT INTO other_expenses (plan_id, desc, amount, icon_code) VALUES (?, ?, ?, ?)`
                );
                otherExpenses.forEach(exp => {
                  expStmt.run(planId, exp.desc, exp.amount, exp.icon_code);
                });
                expStmt.finalize((err) => {
                  if (err) {
                    db.run("ROLLBACK");
                    return res.status(500).json({ error: 'ไม่สามารถเพิ่ม other_expenses: ' + err.message });
                  }
                });
              }

              // Commit transaction เมื่อทุกขั้นตอนสำเร็จ
              db.run("COMMIT", (err) => {
                if (err) {
                  return res.status(500).json({ error: 'Commit transaction failed: ' + err.message });
                }
                res.status(200).json({ message: 'อัปเดตแผนสำเร็จ' });
              });
            });
          });
        });
      }
    );
  });
});

app.delete('/plans/:id', (req, res) => {
  const planId = req.params.id;

  db.serialize(() => {
    db.run(`DELETE FROM favorite_places WHERE plan_id = ?`, [planId]);
    db.run(`DELETE FROM places WHERE plan_id = ?`, [planId]);
    db.run(`DELETE FROM other_expenses WHERE plan_id = ?`, [planId]);
    db.run(`DELETE FROM plans WHERE id = ?`, [planId], function (err) {
      if (err) return res.status(500).json({ error: err.message });
      res.json({ success: true });
    });
  });
});

app.get('/debug/history-khonkaen', (req, res) => {
  db.all(
    `SELECT id, name, category, province FROM all_places WHERE province = 'ขอนแก่น' AND category = 'history'`,
    [],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(rows);
    }
  );
});

app.get('/debug/cafe-khonkaen', (req, res) => {
  db.all(
    `SELECT id, name, category, province FROM all_places WHERE province = 'ขอนแก่น' AND category = 'cafe'`,
    [],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(rows);
    }
  );
});


// --- เริ่มเซิร์ฟเวอร์ ---
app.listen(port, () => {
  console.log(`Server running at http://10.0.2.2:${port}`);

});
